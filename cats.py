...
def pick(paragraphs, select, k):
    ...
    ''
    """
    pgs = [para for para in paragraphs if select(para)==True]
    if k >= len(pgs):
        return ''
    else:
        return pgs[k]
...
def about(subject):
    ...
    def select(para=[]):
        p = split(lower(remove_punctuation(para)))
        for x in p:
            for y in subject:
                if x == y:
                    return True
...
def accuracy(typed, source):
    ...
    if len(typed_words) > len(source_words):
        extra = len(typed_words) - len(source_words)
        typed_words= typed_words[:-extra]

    if len(source_words) > len(typed_words):
        ex = len(source_words) - len(typed_words)
        source_words= source_words[:-ex]
    ...
    return (count/(len(source_words)+extra))*100   
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if min(a, key = abs) > limit:
        return typed_word
    else:
        return word_list[a.index(min(a))]
...
def time_per_word(words, timestamps_per_player):
    ...
    times = []
    for list in timestamps_per_player:
        ti=[]
        for time in range(len(list)-1):
            ti.append(list[time+1] - list[time])
        times.append(ti)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        faster_players.append(fastest_player)    

    nested_list = []
    for k in player_indices:
        player_list=[]
        for h in range(len(faster_players)):
            if faster_players[h] == k:
                player_list.append(get_word(match, h))
